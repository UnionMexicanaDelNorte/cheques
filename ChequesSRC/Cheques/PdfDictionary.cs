...
//					Byte[] ByteArray = Document.TextToByteArray((String) KeyValueItem.Value);

					// encryption is active. PDF string must be encrypted except for encryption dictionary
//					if(Document.Encryption != null && Document.Encryption != Parent)
//						ByteArray = Document.Encryption.EncryptByteArray(Parent.ObjectNumber, ByteArray);

					// convert byte array to PDF string format
//					Str.Append(Document.ByteArrayToPdfString(ByteArray));
